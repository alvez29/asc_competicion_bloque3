...
def algoritmo_total(n, g, porcentaje_t, xl, xu, seed):
    ...
    f = open('ma3_all_p'+str(n)+'g'+str(g)+'_seed'+str(float(seed)).replace('.','')+'.out', 'w')
    ...
    f = open('ma3_plot_p'+str(n)+'g'+str(g)+'_seed'+str(float(seed)).replace('.','')+'.out', 'w')
    for evaluacion_final in evaluar_poblacion(poblacion):
        f.write(str(evaluacion_final[0]) +' '+ str(evaluacion_final[1])+'\n')
    f.close()

    f = open('ma3_EP_p'+str(n)+'g'+str(g)+'_seed'+str(float(seed)).replace('.','')+'.out', 'w')
    for pep in lista_ep:
...
while wi <= 1 :
    if(wi.__round__(1) == 1):
        ...
        algoritmo_total(200, 50 , 0.2, 0, 1, wi.__round__(1))
