    ...
    for i in range(n):
        ...
        for j in range(4):
            if j == 0:
                ...
                aux = random.uniform(-2,2)
            individuo.append(aux)
        poblacion.append(individuo)
    return poblacion

#Evalua la poblacion con la funcion CF6
...
def algoritmo_total(n, g, porcentaje_t, xl, xu, seed):
    ...
    for generacion in range(g):
        for i in range(n):
            ...
            #Selección. Las restricciones se implementan aquí
            restriccion_hijo = cumple_restriccion(hijo)

            for indice_del_vecino in indice_vecinos:
                restriccion_vecino = cumple_restriccion(poblacion[indice_del_vecino])
                
                #Las dos soluciones incumplen las restriccion restricciones
                if restriccion_hijo > 0 and restriccion_vecino > 0:
                    if restriccion_hijo < restriccion_vecino:
                        poblacion[indice_del_vecino] = hijo
                    elif restriccion_hijo > restriccion_vecino:
    ...
    f = open('ma4_plot_p'+str(n)+'g'+str(g)+'_seed'+str(float(seed)).replace('.','')+'.out', 'w')
    for evaluacion_final in evaluar_poblacion(poblacion):
        f.write(str(evaluacion_final[0]) +' '+ str(evaluacion_final[1])+'\n')
    f.close()

    f = open('ma4_EP_p'+str(n)+'g'+str(g)+'_seed'+str(float(seed)).replace('.','')+'.out', 'w')
    for pep in lista_ep:
